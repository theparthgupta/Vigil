...
def _turnover(customer: dict) -> float:
    return float(
        customer.get("stated_monthly_turnover_inr")
        or customer.get("monthly_turnover_inr")
        or 0.0
    )
...
def detect_rapid_passthrough(transactions: list[dict], customer: dict) -> dict:
    ...
    for i, txn in enumerate(txns):
        ...
        if len(new_payees) >= 3:
            ...
            if ratio >= 0.70:
                return _res(True, "rapid_passthrough", 0.85, {
    ...
    return _res(False, "rapid_passthrough", 0.0, {
        "trigger_credit_inr": None, "debit_count": 0,
        "passthrough_ratio": None, "new_payees": [],
    }, ref)
...
def detect_sanctions_hit(transactions: list[dict], customer: dict) -> dict:
    ...
    return _res(bool(hits), "sanctions_hit", 1.0,
                {"hits": hits, "names_checked": len(names)}, ref)
...
def detect_round_trip(transactions: list[dict], customer: dict) -> dict:
    ...
    for i, d in enumerate(txns):
        ...
        for c in txns[i + 1:]:
            ...
            if c["direction"] != "credit":
                continue
            same = (c["counterparty_name"] == d["counterparty_name"]
                    or SequenceMatcher(None, d["counterparty_name"].lower(),
                                       c["counterparty_name"].lower()).ratio() >= 0.85)
            if same and d_amt > 0 and abs(c["amount_inr"] - d_amt) / d_amt <= 0.10:
                return _res(True, "round_trip", 0.75, {
    ...
    return _res(False, "round_trip", 0.0, {
        "counterparty": None, "debit_amount": None,
        "credit_amount": None, "days_elapsed": None,
    }, ref)
...
def detect_velocity_spike(transactions: list[dict], customer: dict) -> dict:
    ...
    return _res(flagged, "velocity_spike", 0.65, {
...
def detect_dormant_reactivation(transactions: list[dict], customer: dict) -> dict:
    ...
    for i in range(1, len(txns)):
        ...
        if gap >= 90 and txns[i]["direction"] == "credit" and txns[i]["amount_inr"] >= _DORMANT_CREDIT:
            window_end = ts[i] + timedelta(days=7)
            debits = [
                txns[j] for j in range(i + 1, len(txns))
                if ts[j] <= window_end and txns[j]["direction"] == "debit"
            ]
            if len(debits) >= 2:
                return _res(True, "dormant_reactivation", 0.65, {
                    "dormancy_days": gap,
                    "reactivation_credit_inr": round(txns[i]["amount_inr"], 2),
                    "rapid_debit_count": len(debits),
                }, ref)

    return _res(False, "dormant_reactivation", 0.0, {
        "dormancy_days": 0, "reactivation_credit_inr": None, "rapid_debit_count": 0,
    }, ref)
...
def detect_smurfing_network(transactions: list[dict], customer: dict) -> dict:
    ...
    for grp in _window(band, 30):
        ...
        if len(senders) >= 3:
            amounts = [t["amount_inr"] for t in grp]
            return _res(True, "smurfing_network", 0.80, {
    ...
    return _res(False, "smurfing_network", 0.0, {
        "sender_names": [], "individual_amounts": [],
        "combined_total_inr": 0.0, "window_days": 30,
    }, ref)
...
def detect_high_risk_sector_spike(transactions: list[dict], customer: dict) -> dict:
    ...
    if btype not in ("jewelry", "real_estate", "logistics") or turnover <= 0:
        return _res(False, "high_risk_sector_spike", 0.0, {
            "stated_turnover_inr": turnover, "actual_volume_inr": 0.0,
            "ratio": 0.0, "business_type": btype,
        }, ref)
    ...
    return _res(flagged, "high_risk_sector_spike", 0.60, {
...
def detect_upi_micro_structuring(transactions: list[dict], customer: dict) -> dict:
    ...
    return _res(False, "upi_micro_structuring", 0.0, {
        "upi_credit_count": 0, "unique_sender_count": 0,
        "total_amount_inr": 0.0, "window_days": 7,
    }, ref)
...
_STATE_MARKERS = {
    "kerala": "Kerala", "tamil": "Tamil Nadu", "chennai": "Tamil Nadu",
    "bengal": "West Bengal", "kolkata": "West Bengal",
    "punjab": "Punjab", "rajasthan": "Rajasthan",
    "gujarat": "Gujarat", "ahmedabad": "Gujarat",
    "marathi": "Maharashtra", "mumbai": "Maharashtra",
    "andhra": "Andhra Pradesh", "telangana": "Telangana", "hyderabad": "Telangana",
    "karnataka": "Karnataka", "bangalore": "Karnataka",
    "odisha": "Odisha", "bihar": "Bihar",
    "up": "Uttar Pradesh", "mp": "Madhya Pradesh", "delhi": "Delhi",
}
...
def detect_geographic_anomaly(transactions: list[dict], customer: dict) -> dict:
    ...
    if not any(states for _, states in tagged):
        return _res(False, "geographic_anomaly", 0.0,
                    {"reason": "state_inference_unavailable"}, ref)
    ...
    for i in range(len(txns)):
        ...
        if len(states) >= 4:
            return _res(True, "geographic_anomaly", 0.55, {
    ...
    return _res(False, "geographic_anomaly", 0.0, {
        "states_detected": [], "state_count": 0, "window_days": 7,
    }, ref)
