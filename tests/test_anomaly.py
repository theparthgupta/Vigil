...
def test_detect_anomaly_shape(cases_by_typology):
    ...
    for key in ("flagged", "typology", "confidence", "evidence",
                "anomaly_score", "regulatory_ref"):
        assert key in out
...
def test_directional_anomaly_scores(cases_by_typology):
    ...
    sus = [detect_anomaly(c, model=model)["anomaly_score"] for c in cases_by_typology["sanctions_hit"]]
